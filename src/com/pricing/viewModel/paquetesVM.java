...
//		System.out.println("hay "+nroDestinosSelect+" destinos");
		...
		}else if(oPaquete.isManejo_propio())
		{
			...
			r=calendarioDao.isOperationCorrect(calendarioDao.generarNuevoCalendarioPropioPaquete(codPaquete,cal.get(Calendar.YEAR)));
	...
	public boolean validoParaInsertar(Component comp)
	{
		boolean valido=true;
		if(!oPaquete.isManejo_camino_inca() &&
				!oPaquete.isManejo_propio() &&
				!oPaquete.isManejo_normal())
		{
			valido=false;
			Clients.showNotification("Debe de Seleccionar algun tipo de manejo para el paquete.", Clients.NOTIFICATION_TYPE_ERROR, comp, "before_start", 2700);
	...
	public void actualizarPaquete(@BindingParam("componente")Component comp,@BindingParam("paquete")CPaquete paquete)
	{
		...
		}else if(paquete.isManejo_propio())
		{
			...
			CCalendarioPropioDAO calendarioDao=new CCalendarioPropioDAO();
			Calendar cal=Calendar.getInstance();
			r=calendarioDao.isOperationCorrect(calendarioDao.generarNuevoCalendarioPropioPaquete(paquete.getcPaqueteCod(),cal.get(Calendar.YEAR)));
